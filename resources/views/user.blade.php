...
<div class="container">
    <div class="text-end m-2"><button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ModalTambahUser"> + add new user</button></div>
    @if(session('message'))
    <div class="alert alert-success m-3"> {{session('message')}} </div>
    @endif
    <table class="table table-secondary table-hover m-lg-3">
        <tr>
            <th>Name</th>
            <th>Email</th>
            <th>Status</th>
        ...
        <tr>
            ...
            <td>
                ...
                @else
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#ModalUpdateUser{{ $p->id }}">Update</button> |
                @endif
        ...
        <div class="modal fade" id="ModalDeleteUser{{$p->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Dalete User</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form action="/user/delete/{{$p->id}}" method="get" class="form-floating">
                            @csrf
                            <div>
                                <h3>Yakin mau menghapus user <b>{{$p->name}}</b> ?</h3>
                            </div>
                            <div class="modal-footer">
